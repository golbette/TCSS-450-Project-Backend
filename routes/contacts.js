...
router.post('/searchfriends', (req, res) => {
    ...
    db.one(`select memberid, username from members where email=$1`, [email]).then(user=>{
        let userid = user.userid;
        let username = user.username;
        db.any(`select memberid, firstname, lastname, email, username from members where memberid = any (select memberid_b from contacts where memberid_a=$1 and verified = 1) or members.memberid = any (select memberid_a from contacts where memberid_b=$1 and verified = 1)`, [userid]).then(contacts=>{
...
=======
        db.any('select members.memberid, members.firstname, members.lastname, members.email, members.username from members where members.memberid = any (select memberid_b from contacts where memberid_a=$1 and verified = 1) or members.memberid = any (select memberid_a from contacts where memberid_b=$1 and verified = 1)', [row.memberid]).then(rows => {
>>>>>>> master
            res.send({
                success:true,
                message:rows
